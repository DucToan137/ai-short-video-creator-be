...
def add_subtitles(video_path, subtitle_path, output_path=None):
    """Add subtitles to a video using FFmpeg with improved error handling"""
    if not output_path:
    ...
    try:
        ...
        # Build FFmpeg command with better subtitle handling
        command = [
            ffmpeg_path,
            "-i", video_path_abs,  # Input video
            "-vf", f"subtitles='{subtitle_path_abs}':force_style='FontSize=16,PrimaryColour=&Hffffff,BackColour=&H80000000'",  # Subtitle filter with styling
        ...
        return add_subtitles_alternative(video_path, subtitle_path, output_path)
        
    except Exception as e:
        print(f"❌ Error adding subtitles: {e}")
        return None

def add_subtitles_alternative(video_path, subtitle_path, output_path):
    ...
    try:
        ...
        # Simpler command without advanced styling
        command = [
            ffmpeg_path,
            "-i", video_path_abs,
            "-vf", f"subtitles={subtitle_path_abs}",  # Simpler subtitle filter
            "-c:a", "copy",
            "-y",
            output_path_abs
        ]
